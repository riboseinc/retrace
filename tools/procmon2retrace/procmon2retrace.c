...
struct ConvState {
	...
	long  pid_filter; /* 0 = keep every pid (TODO.windows/01) */
	int   entries;
	int   bad_rows;
	int   first;
	int   header_done;
...
convert_record(const struct PmCsvRow *row, struct ConvState *st)
{
	...
	/* pid filter: procmon captures are system-wide; scope the
	 * JSON to the traced process at conversion time.
	 */
	if (st->pid_filter != 0 &&
	    st->colmap[PMCOL_PID] >= 0 &&
	    (size_t) st->colmap[PMCOL_PID] < row->count &&
	    atol(row->fields[st->colmap[PMCOL_PID]]) != st->pid_filter)
		return;
...
main(int argc, char **argv)
{
	...
	if (argc < 2 || argc > 5) {
		fprintf(stderr,
			"Usage: procmon2retrace <in.csv> [out.json] [--pid N]\n");
		return 2;
	}
	...
	/* Optional output path first, then --pid N. */
	...
	for (i = 1; i < argc; i++) {
		if (strcmp(argv[i], "--pid") == 0) {
			if (i + 1 >= argc) {
				fprintf(stderr,
					"procmon2retrace: --pid needs N\n");
				return 2;
			}
			st.pid_filter = atol(argv[++i]);
		}
	}
