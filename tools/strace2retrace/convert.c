...
/*
 * strace2retrace: strace log -> retrace trace JSON
 * (TODO.windows/08). The Linux kernel layer (truth) normalizes
 * to retrace's entry shape; retrace-profile --kernel (and
 * retrace-correlate --outside) consume the output like any other
 * stream.
 *
 * Capture with:
 *   strace -f -e trace=%file -o strace.log ./app
 *
 * Recognized line shapes (anything else is skipped):
 *   open("/a/b", O_RDONLY)              = 3
 *   [pid 4212] openat(AT_FDCWD, "/a/b", ...) = 3
 *   stat("/a", 0x7f...)                 = 0
 *   unlink("/a")                        = 0
 * The first quoted string of a file syscall becomes the path;
 * the raw tail becomes "detail" (fopen-style mode flags feed the
...
/*
 * strace escapes special bytes inside quoted strings. Unescape
...
/*
 * Parse one strace line. Returns 1 when an entry was appended to
...
static int convert_line(const char *line, JSON_Array *out)
{
	...
	/* pid prefix: "[pid N] " (stderr) or "N  " (-f -o file) */
	if (p[0] == '[' && strncmp(p, "[pid ", 5) == 0) {
		pid = strtol(p + 5, (char **)&p, 10);
		if (*p == ']')
			p++;
	} else if (p[0] >= '0' && p[0] <= '9') {
	...
	if (name_len == 0 || !is_file_func(name, name_len))
		return 0;
	...
	/* flags only: from after the path to the next ',' ')' '=' */
	{
		...
		/* structured args (argv, stat buffers) are not flags */
		if (*start != '[' && *start != '{') {
			while (*end != '\0' && *end != ',' &&
			       *end != ')' && *end != '=')
				end++;
	...
	json_object_set_string(entry_o, "module", "strace");
