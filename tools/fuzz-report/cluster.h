...
struct FuzzCluster {
	...
	unsigned long coverage; /* the call-hash id (0 = none) */
 ...
 * coverage: the iteration's call-hash id (RETRACE_CALL_HASH's
 * per-thread hashes, combined); 0 = the lane carried none
 * (legacy behavior: coverage never separates).
