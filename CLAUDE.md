...
- **Backend plugin system** (`src/backends/`): each (OS, arch) combo has its own backend (`preload_elf`, `preload_macho`, `preload_bsd`, `preload_msvc`, `preload_mingw`, `ptrace`). Backends self-register via constructor-section scan; `retrace_backend_select()` picks the highest-rank backend whose `probe()` succeeds.
