...
- **Function inventory**: the shared list is `src/v2/funcs_symbols.def`; each backend's `funcs_symbols.S` includes it and adds platform extras. `unit-test-inventory-conformance` enforces that every backend covers the whole `.def` (the v2.36.0 linux-aarch64 drift lesson). Add a new function by adding a line to the `.def` **and** a prototype entry below.
...
- **Live OTLP + the log-sink seam** (`src/core/otlp_live.{c,h}`, `src/core/otlp_allocator.{c,h}`): `RETRACE_OTLP_ENDPOINT` streams spans/security events live (Waves B/C). The logger's flusher fans drained entries out to registered sinks (`retrace_log_sink_register`, `logger.h`) — otlp_live is the first subscriber; feature modules subscribe instead of editing logger.c. The size-header allocator shim that fronts otlp-c is its own unit-tested module (`otlp_allocator.c`) — reuse it whenever a module links otlp-c from inside the traced process.
...
- **As-ops seam** (ADR-0016, `src/backends/as_ops.c`): the engine drives every frame through the five `retrace_as_*` verbs, dispatched via a thread-local ops table. The preload backends publish `retrace_as_ops_default` (the trampoline frame); the ptrace backend's `as_ptrace.c` publishes the syscall-lane table (the kernel IS the real impl: denial returns `-errno`, `call_real` = allow). New lanes = new ops table, no engine change.
...
| `src/core/otlp_live.{c,h}` / `otlp_allocator.{c,h}` | Live OTLP streaming + its allocator shim; the log-sink seam (`retrace_log_sink_register`) is in `logger.h` |
